...
假设你有一个大小为 N 的数组。我们可以倒推计算出每次扩容时需要复制的元素数量。可以看到，当我们将数组增加到 K 个元素时，该数组之前的大小就是这个数字的一半。因此，我们需要复制 $\frac{K}{2}$ 个元素。
...
因此，插入N个元素的复制总数大致为 $\frac{N}{2} + \frac{N}{4}  + \frac{N}{8}  + ... + 2 + 1$，刚好小于 N。
